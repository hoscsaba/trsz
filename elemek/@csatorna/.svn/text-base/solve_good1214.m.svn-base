...
y_value_beg = max([y_akna_beg   Ytr(1)-g*dt^2/2   -Vtr(1)*yc_beg/max(abs(Vtr(1)),1)   y_akna_beg   X*0.001]);
...
for k=1:N
    ...
    svs(k)=g*get_A(csatorna,Ytr(k))/get_B(csatorna,Ytr(k));  
end
...
for k=2:N-1
    ...
    Aeq(N+k,k)  =0;         Aeq(N+k,N+k)  =1/dt+g*n^2*abs(Vtr(k))/get_Rh(csatorna,Ytr(k))^(4/3);
    Aeq(N+k,k+1)=g/dx/2;    Aeq(N+k,N+k+1)=Vtr(k)/dx/2;
...
%% Megoldas
[mo flag]=bicgstab(Aeq,Beq,1e-6,[],[],[],[Ytr(1,:), Vtr(1,:)]');
...
Dyj=Ytr(3:end)-Ytr(2:end-1); Dyb=Ytr(2:end-1)-Ytr(1:end-2);
Y_mean=(Ytr(3:end)+Ytr(2:end-1)+Ytr(1:end-2))/3; DY=Y_mean-Ytr(2:end-1);
...
Dvj=Vtr(3:end)-Vtr(2:end-1); Dvb=Vtr(2:end-1)-Vtr(1:end-2);
V_mean=(Vtr(3:end)+Vtr(2:end-1)+Vtr(1:end-2))/3; DV=V_mean-Vtr(2:end-1);
