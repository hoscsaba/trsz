function csatorna = set(csatorna,varargin)
...
while length(property_argin) >= 2,
  ...
  switch prop
   ...
   case 'hely'
	...
	DD(i,i-2)=1; DD(i,i-1)=-8; DD(i,i+2)=-1; DD(i,i+1)=8;
     ...
     case 'femi'
      ...
      csatorna.DD=[zeros(N+1,N+1) DD1; DD2 zeros(N+1,N+1)];
      
     otherwise
      fprintf('\n\n ERROR!! -> Unknown spatial disretization in %s',csatorna.name);
   ...
   case 'reltol'
    csatorna.reltol = val;
   case 'friction'
    csatorna.friction = val;
    switch csatorna.friction
     case 'unsteady'                    
      switch csatorna.hely
       case 'moc'
	for i=1:csatorna.N+1
	  csatorna.vregi(i)=csatorna.v(i);
	  for j=1:csatorna.zielke_term
	    csatorna.I(i,j)=0;
	  end      
	end
      end
