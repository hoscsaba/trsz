...
for k=2:N-1
    ...
    if Xmesh(k)<xP(k)
        ...
        yPint(k)=(yP(k)-yL)*(t_int(k)-t)/(tP(k)-t)+yL;
        vPint(k)=(vP(k)-vL)*(t_int(k)-t)/(tP(k)-t)+vL;
    else
        t_int(k)=(Xmesh(k)-Xmesh(k+1))*(tP(k)-t)/(xP(k)-Xmesh(k+1))+t;
        yPint(k)=(yP(k)-yR)*(t_int(k)-t)/(tP(k)-t)+yR;
        vPint(k)=(vP(k)-vR)*(t_int(k)-t)/(tP(k)-t)+vR;
    end
end
tmin=min(t_int(2:end-1)); T=tmin; dt=T-t;
for k=2:N-1
