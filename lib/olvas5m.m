...
%                    fprintf('\n\t %2d./%2d adat:  t=%+5.3e  eps=%5.3e',j,jgpszt(k),t_vf(k,j),epst(k,j));
		...
		% Ez már közös...
            ...
            case 'szivattyu'
                ...
                for j=1:jgpsz(k)
                    ...
                    H(k,j)=str2num(char(be(eltol+jj+1))); %...szállitomagassag [m]
        ...
        % Itt a nagy ciklus jc-je felülíródik, ami szeintem hibás. A belsõ
        ...
        for k=1:nag 
	        ...
	        case 'vez_fojtas' 
                ...
                otherwise
                    error(['olvas5m -> Ismeretlen ágelem: ',tipus{k}])
            end
        end
        
        if debug>1, fprintf('   OK\n    merev alrendszer építése...'); end;
        clear csp;
        maxj=length(csom(1,:));
        for i=1:ncp_m
            j=1; clear agak;
