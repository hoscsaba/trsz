...
switch nargin
    ...
    case 9
        ...
        dfyy = varargin{8};
        teltol = varargin{9};
        dataeltol = varargin{10};
        dataszorzo= varargin{11};
        fdata=load(dfnev);
        % fdata=fdata';
...
switch restipus
    case 'mar'
        data=load(fullfile(wdir.path,strcat(mar{melyik1}.nev,'.res')));
        switch mit
            % nyomas csomopontban
            case 'p'
            ...
            case 'Qm3/h'
                ...
                if van_df==0
                    ...
                    figure
                    plot(data(:,1),data(:,1+eltol)*3600,dfx,dfy*3600);
                    xlabel('t [s]'), ylabel('Q [m^3/h]'), grid on
                    title(['Terfogataram a ',mar{melyik1}.nev,' merev alrendszer ',hol2,' agaban']);
                    legend('Szimulacio',strcat('Adatfile: ',dfnev));
                    varargout=compute_errors(data(:,1),data(:,1+eltol),dfx,dfy);
            ...
            otherwise
                error('Ismeretlen opcio merev alrendszerre (p,Q,Qm3/h,m,n)')
