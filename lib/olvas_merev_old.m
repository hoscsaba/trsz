function mar = olvas_merev(fnev,debug,wdir)
        ...
        switch tipus{k}
            ...
            case 'szivattyu'
                ...
                for j=1:jgpsz
                    ...
                    H(j)=str2num(char(be(eltol+jj+1))); %...sz�llitomagassag [m]
                ...
                switch tranziens(k)
                    ...
                    case 4     %...frekivaltos inditas, kozelito motor-jelleggorbe...
                        ...
                        Mb(k)  = str2num(char(be(eltol+3)));         %...motor billeno nyomatek [Nm]
                        nb(k)  = str2num(char(be(eltol+4)));         %...motor billeno-nyomatek fordulatszama [1/min]
                        Mi(k)  = str2num(char(be(eltol+5)));         %...motor indít�-nyomatek [Nm]
                        nsz(k) = str2num(char(be(eltol+6)));         %...motor szinkron-fordulatszama [1/min]
                        tbe(k) = str2num(char(be(eltol+7)));         %...indít�s időpontja [s]
                        tind(k) = str2num(char(be(eltol+8)));        %...felfutas idotartama [s]
                        eltol=eltol+9;
                    case 5
                        n(k)=str2num(char(be(eltol+1)));            %...fordulatszam [1/min]
                        hbe(k)=str2num(char(be(eltol+2)));          %...bekapcsolasi szint [m]
                        hki(k)=str2num(char(be(eltol+3)));          %...kikapcsolasi szint [m]
                        uzem(k)=str2num(char(be(eltol+4)));         %...szivattyu uzemel (0/1)
                        eltol = eltol + 5;
                    otherwise
            ...
            case 'akna'
                ...
                hmin(k) =str2num(char(be(eltol+4)));
                hmax(k) =str2num(char(be(eltol+5)));
                yy0(k)  =str2num(char(be(eltol+6)));
                ro(k)  =str2num(char(be(eltol+7)));
                m0(k)  =str2num(char(be(eltol+8)));
                rajz{k}  =be(eltol+9);
                eltol=eltol+10;
                if debug>1, fprintf('\n   %2d. elem: akna (%s)',ag_szamlalo,agnev{k}); end  
            ...
            case 'nyomovezetek'
                ...
                L(k)=str2num(char(be(eltol+5)));            %...aghossz [m]
                lam(k)=str2num(char(be(eltol+6)));          %...lambda
                ze(k)=str2num(char(be(eltol+7)));           %...cso eleje magassag [m]
                zv(k)=str2num(char(be(eltol+8)));           %...cso vege magassag [m]                
                ro(k)=str2num(char(be(eltol+9)));           %...suruseg [kg/m3]
                m0(k)=str2num(char(be(eltol+10)));          %...tomegaram [kg/s]

                eltol=eltol+11;
                if debug>1, fprintf('\n   %2d. elem: nyomovezetek (%s)',ag_szamlalo,agnev{k}); end
