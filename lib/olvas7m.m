function mar = olvas7m(fnev,debug)
...
% be string vektorba beolvassuk az egész file-t.
[be] = textread(fnevdat,'%s','delimiter',',');

%...k: cso-index...j: jelleggorbe-index...jc: csomopont index...
%..feladat neve
fnev = char(be(1));
ncp = str2num(char(be(2)));       %....beolvasandó merev alrendszerek száma...
eltol = 3;
mar_szamlalo=0;
for jc=1:ncp
    ag_szamlalo=0;
    ...
    if debug>1, fprintf('\n\n%2d./%2d csomoponti alrendszer neve: %s  (%d ág)',mar_szamlalo,ncp,cspnev,nag); end
        ...
        switch tipus{k}
            ...
            case 'szivattyu'
                ...
                for j=1:jgpsz
                    ...
                    H(j)=str2num(char(be(eltol+jj+1))); %...szállitomagassag [m]
                ...
                switch tranziens(k)
                    ...
                    case 4     %...frekivaltos inditas, kozelito motor-jelleggorbe...
                        ...
                        Mb(k)  = str2num(char(be(eltol+3)));         %...motor billeno nyomatek [Nm]
                        nb(k)  = str2num(char(be(eltol+4)));         %...motor billeno-nyomatek fordulatszama [1/min]
                        Mi(k)  = str2num(char(be(eltol+5)));         %...motor indÃ­tó-nyomatek [Nm]
                        nsz(k) = str2num(char(be(eltol+6)));         %...motor szinkron-fordulatszama [1/min]
                        tbe(k) = str2num(char(be(eltol+7)));         %...indÃ­tás idÅ‘pontja [s]
            ...
            case 'nyomasszabalyzo'
                ...
                switch szab{k}
                    ...
                    case 'dp'
                        ...
                        eltol = eltol+12;
                    otherwise
                        error('Szabályzószelep csak nyomásra  vagy nyomáskülönbségre tud szabályozni!');
                end

                if debug>1, fprintf('\n   %2d. elem: nyomásszabályzó szelep (%s)',ag_szamlalo,agnev{k}); end
            ...
            case 'legust'
                ...
                eltol=eltol+11;
                if debug>1, fprintf('\n   %2d. elem: légüst (%s)',ag_szamlalo,agnev{k}); end
            ...
            case 'akna'
                ...
                csp1{k}  =char(be(eltol+2));
                csp2{k}='';
                AA(k)   =str2num(char(be(eltol+3)));
                yy0(k)  =str2num(char(be(eltol+4)));
                pp0(k)  =str2num(char(be(eltol+5)));
                ro(k)  =str2num(char(be(eltol+6)));
                m0(k)  =str2num(char(be(eltol+7)));
                eltol=eltol+8;
                if debug>1, fprintf('\n   %2d. elem: légüst (%s)',ag_szamlalo,agnev{k}); end                
    ...
    % Itt a nagy ciklus jc-je felülÃ­ródik, ami szeintem hibás. A belsÃµ
    % jc-ket lecserélem jcc-re.
    cpnevsor{1}=csp1{1};
    jcc=1;
    for k=1:nag
        jel='n';
    ...
    % Itt szintén lecseréltem a jc-t jcc-re. HCs
    rang=zeros(1,ncp_m);
    for jcc=1:ncp_m
        mar_volt=0;
        for k=1:nag
    ...
    for jcc=1:ncp_m
        ...
        if strcmp(jelzo,'nincs')
            fprintf('\n\n %8s nevÃ» csomópont nincs\n\n',cspnev1{jcc}); pause
        end

    end

    if debug>2
        fprintf('\n\n    szám    név        z[m]    fogy.[kg/s]');
        for jcc=1:ncp_m
    ...
    for k=1:nag
        ...
        switch tipus{k}
            ...
            case 'akna'
                elemek{k}=akna(agnev{k},cspe_szam,AA(k),yy0(k),pp0(k),ro(k),m0(k));
                
            otherwise
                error(['olvas7m -> Ismeretlen ágelem: ',tipus{k}])
        end
    end
